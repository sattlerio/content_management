    ...
    <div class="col-lg-12 col-md-12 col-sm-12">
      <div class="card">
        <div class="card-header">
          ...
          <h4 class="title pull-left">Edit Tax Rule {{ tax_rule.name }} for Tax {{ tax.name }} </h4>
  ...
  export default {
    ...
    methods: {
      ...
      fetchData: function () {
        ...
        this.axios.get('/api/accounting/tax/rule/configuration/data/' + routeParams.tax_id + '/' + routeParams.company_id + '?tax_rule_id=' + routeParams.tax_rule_id)
          .then(function (response) {
        ...
        this.axios.get('/api/accounting/tax/rule/' + routeParams.tax_rule_id + '/' + routeParams.tax_id + '/' + routeParams.company_id)
          .then(function (response) {
            var taxData = response.data.data
            self.tax_rule.name = taxData.tax_rule_name
            self.tax_rule.value = taxData.value
            self.tax_rule.b2c_rule = taxData.b2c_rule

            if (taxData.b2c_rule) {
              self.tax_rule.b2c_countries = taxData.countries
            } else {
              self.tax_rule.countries = taxData.countries
            }
            self.loading = false
          })
          .catch(function (error) {
        ...
        this.$validator.validateAll()
          .then((result) => {
            ...
            if (result) {
              ...
              self.axios.put('/api/accounting/tax/' + self.$route.params.tax_id + '/create/rule/' + self.$route.params.company_id + '?tax_rule_id=' + self.$route.params.tax_rule_id, data, {
                headers: {
